...
public protocol PSImageRequestCache: PSImageCache {
    /// Adds the image to the cache using an identifier created from the request and identifier.
    func add(_ image: UIImage, for request: URLRequest, withIdentifier identifier: String?)
    ...
    func image(for request: URLRequest, withIdentifier identifier: String?) -> UIImage?
}
...
open class PSAutoPurgingImageCache: PSImageRequestCache {
    // MARK: Properties
    ...
    private var cachedImages: [String: PFCachedImage]
    ...
    public init(memoryCapacity: UInt64 = 100_000_000, preferredMemoryUsageAfterPurge: UInt64 = 60_000_000) {
        ...
        NotificationCenter.default.addObserver(self,
                                               selector: #selector(PSAutoPurgingImageCache.removeAllImages),
                                               name: notification,
                                               object: nil)
        #endif
    }
    ...
    open func add(_ image: UIImage, for request: URLRequest, withIdentifier identifier: String? = nil) {
    ...
    open func add(_ image: UIImage, withIdentifier identifier: String) {
        synchronizationQueue.async(flags: [.barrier]) {
            let cachedImage = PFCachedImage(image, identifier: identifier)
    ...
    open func image(for request: URLRequest, withIdentifier identifier: String? = nil) -> UIImage? {
        let requestIdentifier = imageCacheKey(for: request, withIdentifier: identifier)
